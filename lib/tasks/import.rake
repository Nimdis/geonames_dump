...
namespace :geonames_dump do
  namespace :import do
    ...
    task :alternate_names => [:prepare, :environment] do
      ...
      write = -> klass, attributes {
        if klass == GeonamesAlternateName
          country = GeonamesCountry.where(geonameid: attributes[:geonameid]).first
          city = GeonamesCity.where(geonameid: attributes[:geonameid]).first
          return country.present? || city.present?
        end
        return true
      }
      File.open(txt_file) do |f|
    ...
    task :admin1 => [:prepare, :environment] do
      ...
      change_type = -> klass, attributes, col_value, idx {
      ...
      File.open(txt_file) do |f|
        insert_data(f, GEONAMES_ADMINS_COL_NAME, GeonamesAdmin1,
                    title: "Admin1 subdivisions",
    ...
    task :admin2 => [:prepare, :environment] do
      ...
      File.open(txt_file) do |f|
        insert_data(f, GEONAMES_ADMINS_COL_NAME, GeonamesAdmin2,
                    title: "Admin2 subdivisions",
                    change_type: change_type)
      end
    end
    ...
    def insert_data(file_fd, col_names, main_klass = GeonamesFeature, options = {})
      ...
      file_fd.each_line do |line|
        ...
        line.strip.split("\t").each_with_index do |col_value, idx|
          ...
          # lambda may change the type of object to create
          if options[:change_type].present?
            options[:change_type].call klass, attributes, col_value, idx
          else
        ...
        blocks.add_block do
          ...
          if primary_keys.all? { |key| attributes.include?(key) }
            if ENV['QUICK']
              ...
              where_condition = {}
              if options[:write?].present?
                next unless options[:write?].call klass, attributes
              end
